...
int main( int argc, char *argv[] )
{
  ...
  if ( pid == 0 ) {
    ...
    const char *lang_env_list[] = { "LANG", "LANGUAGE", "LC_CTYPE", "LC_NUMERIC", "LC_TIME", "LC_COLLATE", "LC_MONETARY", "LC_MESSAGES", "LC_PAPER", "LC_NAME", "LC_ADDRESS", "LC_TELEPHONE", "LC_MEASUREMENT", "LC_IDENTIFICATION", "LC_ALL", NULL };
    const int lang_env_length = 15;
    for ( int i = 0; i < lang_env_length && lang_env_list[i] != NULL; i++ ) {
      char const *lang_env_value = getenv( lang_env_list[i] );
      if ( lang_env_value != NULL ) {
        string remote_env = string( lang_env_list[i] ) + "=" + string( lang_env_value );
        server_args.push_back( "-l" );
        server_args.push_back( remote_env );
      }
    }
